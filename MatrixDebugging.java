...
public class MatrixDebugging {
    public static void main(String[] args) {
        ...
        // Crear la matriu tombada 90 graus
        int[][] matriuTombada = new int[matrix[0].length][matrix.length];
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                matriuTombada[i][j] = matrix[(matrix.length - 1) - j][i];
            }
        }

        // Mostrar la matriu tombada 90 graus
        System.out.println("\nMatriu +90 graus:");
        for (int i = 0; i < matriuTombada.length; i++) {
            for (int j = 0; j < matriuTombada[i].length; j++) {
                System.out.print(matriuTombada[i][j] + " ");
            }
            System.out.println();
        }
